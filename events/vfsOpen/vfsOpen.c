...
int BPF_KPROBE(vfs_open, const struct path *path, struct file *file) {
    ...
    // TODO: Later, we'll concatenate each filepath into a full path.
    for (i = 0; i < 50 && dentry != NULL; i++) {
        ...
        bpf_probe_read_kernel_str(e->filepath, sizeof(e->filepath), fname_ptr);

        // Discard if e->filepath is "/"
        if (e->filepath[0] == '/' && e->filepath[1] == '\0') {
            bpf_ringbuf_discard(e, 0);
            break;
        }
